...
export class RegisterComponent implements OnInit {
    ...
    this.RegisterForm = this.fb.group({
     ...
     this.authService.getAllNationalites().subscribe({
      next: (data) => {
        ...
        console.log('Nationalite:', this.nationalites);

            },
            error: (err) => console.error('Erreur : nationalité introuvable', err)
  ...
  onSubmit(): void {
    ...
    let payload: any = {
      ...
      roleId: 2,                       
      nationaliteId: Number(formValue.nationaliteId),  
      phone: formValue.telephone,
      typePersonne: formValue.type      
    };
    ...
    this.authService.register(payload).subscribe({
      next: () => {
        ...
        sessionStorage.setItem("verifyEmail", formValue.email);
